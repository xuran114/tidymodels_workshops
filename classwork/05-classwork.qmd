...
# Your code here!
nhl_train %>% 
  group_by(position, on_goal) %>% 
  tally()

nhl_train %>% 
  ggolot(aes(x = position, color = on_goal)) +
  geom_bar()
...
nhl_indicators <-
  ...
  step_dummy(all_nominal_predictors()) %>%
  step_zv(all_predictors()) %>% 
  step_lincomb(all_numeric_predictors()) # to avoid warnings for linear dependency in regression model
...
nhl_effect_rec <-
  recipe(on_goal ~ ., data = nhl_train) %>%
  step_lencode_mixed(player, outcome = vars(on_goal)) %>%  # make sure this step is before step_dummy()
  step_dummy(all_nominal_predictors()) %>%
  step_zv(all_predictors())
```
...
## Recipies prep() and bake()

```{r}
fitted_rec <- nhl_effect_rec %>% prep(nhl_train)

fitted_rec %>% bake(new_data = nhl_train %>% slice(1:3))

fitted_rec %>% bake(new_data = nhl_train %>% slice(1:3), starts_with("coord"))

fitted_rec %>% bake(new_data = nhl_train %>% slice(1:3), starts_with("coord"), composition = "matrix")

fitted_rec %>% bake(new_data = nhl_train %>% slice(1:3), starts_with("coord"), composition = "dgCMatrix")

fitted_rec %>% tidy(number = 1)
