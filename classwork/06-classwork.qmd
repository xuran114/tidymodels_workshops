...
```{r}
# Your code here!
select_by_pct_loss(glm_spline_res, distance, metric = "roc_auc") # simplest model by distance (smallest 'distance' in this case) that makes  "roc_auc" not worse than the best model by a default limit
select_by_pct_loss(glm_spline_res, distance, limit = .4, metric = "roc_auc") # simplest model by distance (smallest 'distance' in this case) that makes  "roc_auc" not worse than the best model by .4% limit
```
...
predict(final_glm_spline_wflow, nhl_test[1:3,], type = "prob")
predict(final_glm_spline_wflow, nhl_test[1:3,], type = "conf_int")
```
